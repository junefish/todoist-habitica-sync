...
class TestChangelog:
    ...
    def should_use_latest_version_in_the_unreleased_section_link(poetry, changelog):
        regex = r"\[Unreleased\]: " + poetry.homepage + \
            r"/compare/" + poetry.version + r"\.\.\.HEAD"
    ...
    def should_link_to_the_current_version(poetry, changelog):
        regex = r"\[" + poetry.version + r"\]: " + \
            poetry.homepage + r"/compare/.*\.\.\." + poetry.version
        links = re.findall(regex, changelog)
        assert len(
            links) == 1, f"Pattern '{regex}' must be present exactly once."
    ...
    def should_contain_change_type_in_third_level_headings(changelog):
        ...
        assert headings <= {"Features", "Fixes",
