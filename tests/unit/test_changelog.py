...
def changelog() -> str:
    with open(Path(__file__).parent.parent.parent / "CHANGELOG.md",
              "r",
              encoding="utf8") as changelog:
...
class TestChangelog:
        ...
        for version, count in cnt.most_common():
            if count > 1:
                raise AssertionError(
                    f"Version '{version}' appears {count}x in the changelog.")
            break

    @staticmethod
    def should_use_latest_version_in_the_unreleased_section_link(
            poetry, changelog):
        regex = r"\[Unreleased\]: " + poetry.homepage + r"/compare/" + poetry.version + r"\.\.\.HEAD"
        links = re.findall(regex, changelog)
        assert len(
            links) == 1, f"Pattern '{regex}' must be present exactly once."
    ...
    @staticmethod
    def should_contain_version_and_date_in_add_second_level_headings(
            changelog):
    ...
    def should_contain_change_type_in_third_level_headings(changelog):
        ...
        assert headings <= {
            "Features", "Fixes", "Deprecated", "Breaking changes"
        }
