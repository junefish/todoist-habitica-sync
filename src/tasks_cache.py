...
class TasksCache:
    ...
    HABITICA_DIRTY_STATES = frozenset([
        TaskState.HABITICA_NEW, TaskState.HABITICA_CREATED,
        TaskState.HABITICA_FINISHED
    ])
    ...
    def get_task_by_todoist_task_id(
            self, todoist_task: TodoistTask) -> Optional[GenericTask]:
    ...
    def set_habitica_id(self, generic_task: GenericTask,
                        new_habitica_task_id: str):
    ...
    def dirty_habitica_tasks(self) -> Iterator[GenericTask]:
        condition = Query().state.test(
            lambda _: _ in TasksCache.HABITICA_DIRTY_STATES)
